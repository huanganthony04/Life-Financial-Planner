...
const ExpenseEvent = ({ scenarioId, scenarioName }) => {
  const navigate = useNavigate();

  // State hooks
  const [title, setTitle] = useState("");
  const [summary, setSummary] = useState(""); // optional
  ...
  const [startsWith1, setStartsWith1] = useState(""); // optional
  ...
  const handleDiscretion = () => setDiscretionary(!discretionaryStatus);
  ...
  return (
    <div className="expense-event-container">
      <h1>Create Expense Event</h1>
      ...
      <ExpenseEventList scenarioId={scenarioId} />
    </div>
  );
};
